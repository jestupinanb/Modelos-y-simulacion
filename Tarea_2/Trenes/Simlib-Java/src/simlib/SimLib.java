...
public class SimLib {
	...
	public static void timing_sin_prioridad() {
	...
	public static void timing() {
		...
		float tempTime = simTime;
		Event event;
		Event event_type_2 = null;
		Event event_type_3 = null;
		do {
			event = events.remove();
			if (event.getType() == 2 && event_type_2 == null) {
				event_type_2 = event;
			} else if (event.getType() == 3 && event_type_3 == null) {
				event_type_3 = event;
			} else {
				removed.add(event);
			}
		} while (simTime == event.getTime());
		//if (!events.isEmpty()) {
			if (event_type_2 != null) {
				// programar salida
				simTime = event_type_2.getTime();
				eventType = event_type_2.getType();
				eventAttributes = event_type_2.getAttributes();
				
				if (event_type_3 != null) {
					removed.add(event_type_3);
				}
				events.addAll(removed);
			} else if (event_type_3 != null) {
				// programar salida
				simTime = event_type_3.getTime();
				eventType = event_type_3.getType();
				eventAttributes = event_type_3.getAttributes();
	 ...
	 */
	/*
	 * public static boolean removeEvent( float time ){ HashSet<Event> removed = new
	 * HashSet<>(); while( !events.isEmpty() && events.element().getTime() <= time
	 * ){ Event event = events.remove(); if( event.getTime() != time ) removed.add(
	 * event ); } events.addAll( removed ); return !removed.isEmpty(); }
	 */

	public static boolean removeEventType(int id) {
		HashSet<Event> removed = new HashSet<>();
		while (true) {
			Event event = events.remove();
			if (event.getType() == 2) {
				if (event.getAttribute(0) == id) {
					break;
				} else {
					removed.add(event);
				}
	...
	private static long zrng[] = { 0, 1973272912, 281629770, 20006270, 1280689831, 2096730329, 1933576050, 913566091,
			246780520, 1363774876, 604901985, 1511192140, 1259851944, 824064364, 150493284, 242708531, 75253171,
			1964472944, 1202299975, 233217322, 1911216000, 726370533, 403498145, 993232223, 1103205531, 762430696,
			1922803170, 1385516923, 76271663, 413682397, 726466604, 336157058, 1432650381, 1120463904, 595778810,
			877722890, 1046574445, 68911991, 2088367019, 748545416, 622401386, 2122378830, 640690903, 1774806513,
			2132545692, 2079249579, 78130110, 852776735, 1187867272, 1351423507, 1645973084, 1997049139, 922510944,
			2045512870, 898585771, 243649545, 1004818771, 773686062, 403188473, 372279877, 1901633463, 498067494,
			2087759558, 493157915, 597104727, 1530940798, 1814496276, 536444882, 1663153658, 855503735, 67784357,
			1432404475, 619691088, 119025595, 880802310, 176192644, 1116780070, 277854671, 1366580350, 1142483975,
			2026948561, 1053920743, 786262391, 1792203830, 1494667770, 1923011392, 1433700034, 1244184613, 1147297105,
			539712780, 1545929719, 190641742, 1645390429, 264907697, 620389253, 1502074852, 927711160, 364849192,
